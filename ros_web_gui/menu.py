...
def get_items(data, active_item=None):
    ...
    # Navigation
    nav_node_items =    [{'href': url_for('node.get_node_info', name=node), 'caption': node} for node in node_names]
    nav_topic_items =   [{'href': url_for('topic.get_topic_info', name=topic), 'caption': topic} for topic in topic_names]
    nav_service_items = [{'href': url_for('service.get_service_info', name=service), 'caption': service} for service in service_names]
